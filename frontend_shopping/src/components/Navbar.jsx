...
export default function Navbar({ hydrated ,setActiveCategory }) {
  ...
  const cartCount = token && !hydrated ? null : useSelector((state) => state.cartItem.cart?.length || 0);
  const wishlistCount = token && !hydrated ? null : useSelector((state) => state.wishlistItem.wishlist?.length || 0);
